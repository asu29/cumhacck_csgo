...
bool interfaces::initialize() 
{
	...
	directx = **(IDirect3DDevice9***)(utilities::pattern_scan(GetModuleHandleA(xorstr_("shaderapidx9.dll")), xorstr_("A1 ? ? ? ? 50 8B 08 FF 51 0C")) + 1);
	input = *(i_input**)(utilities::pattern_scan(GetModuleHandleA(xorstr_("client.dll")), xorstr_("B9 ? ? ? ? F3 0F 11 04 24 FF 50 10")) + 1);
	glow_manager = (glow_manager_t*)(*(uintptr_t*)(utilities::pattern_scan(GetModuleHandleA(xorstr_("client.dll")), xorstr_("0F 11 05 ? ? ? ? 83 C8 01 C7 05 ? ? ? ? 00 00 00 00")) + 3));
	move_helper = **(player_move_helper***)(utilities::pattern_scan(GetModuleHandleA(xorstr_("client.dll")), xorstr_("8B 0D ? ? ? ? 8B 46 08 68")) + 2);
	weapon_system = *(i_weapon_system**)(utilities::pattern_scan(GetModuleHandleA(xorstr_("client.dll")), xorstr_("8B 35 ? ? ? ? FF 10 0F B7 C0")) + 2);

	mem_alloc = *(i_mem_alloc**)(GetProcAddress(GetModuleHandleA(xorstr_("tier0.dll")), xorstr_("g_pMemAlloc")));
